...
Также можно отправлять деньги непосредственно через консоль ноды блокчейна (подробнее [здесь](#cmd)) 
...
**0xe7b0600cd184432527b3ea401eebb5dc5d05b855** 
А именно: добавить файл с ключом [JSON](https://github.com/pleshakoff/blockchain-casino/blob/master/geth/UTC--2020-02-23T03-54-10.906285725Z--1ad4f70b6a49d0448c1f1392db93793bff540e2b).
...
Также к кошельку можно подключить наши коины и тогда будет видно сколько их у пользователя. 
...
<a name="cmd"></a>
### Работа с консолью ноды 

Осуществлять переводы между счетами можно также через консоль ноды блокчейна. 

Подключаемся к контейнеру с нодой 

  ```
docker exec -it bc-geth bash
  ```
Подключаемся к консоли ноды   
  
  ```
geth attach  
  ```
Разблокируем тестовый адрес. На продакшене такое сделать не получиться.
Но в текущей реализации эта возможность разблокирована. В качестве параметров, адрес и пароль.     
  ```
  web3.personal.unlockAccount("1ad4f70b6a49d0448c1f1392db93793bff540e2b","12345")
  ```
Если все правильно возвращает значение true
    
Переводим эфир с тестового адреса на адрес смарт-контракта(на тестовом адресе 100 ETH, 
адрес смарт-контракта здесь http://localhost:8080/api/v1/address) 
    
  ```
  eth.sendTransaction({from:"1ad4f70b6a49d0448c1f1392db93793bff540e2b",to:"здесь адрес смарт-контракта",value:web3.toWei(30, "ether")})
  ```
Через приложение проверяем что эфир на адрес смарт-контракта пришел (можно через swagger)

http://localhost:8080/api/v1/blockchain/balance/ЗдесьАдресСмартКонтракта

Проверяем что на баланс в казино пришли коины на счет нашего тестового адреса 
...
Чтобы выйти из коинов и вывести эфир, надо выполнить метод 
**POST** http://localhost:8080/api/v1/withdraw/0x1ad4F70b6a49d0448C1f1392db93793BFF540E2b

Помните, у казино нет своих денег, если играете один то вывести больше чем ввели не получится.
